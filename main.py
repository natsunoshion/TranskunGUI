...
def main(page: ft.Page):
    ...
    def run_transkun_task():
        try:
            ...
            device = "cuda" if use_cuda.value else "cpu"
            ...
            try:
                ...
                # 模型文件不存在的错误
                error_message = str(e)
                status_text.value = "转换失败：找不到模型文件"
                            ...
                            [
                                ft.Row(
                                    [
                                        ...
                                        ft.Text("错误：找不到模型文件", weight=ft.FontWeight.W_500)
                                    ],
                                    spacing=10
                                ),
    ...
    # CUDA status and option
    cuda_status = ft.Text(
        f"CUDA {'可用 ✓' if cuda_available else '不可用 ✗'}",
        color=ft.Colors.GREEN if cuda_available else ft.Colors.RED_400,
        weight=ft.FontWeight.W_500
    )

    use_cuda = ft.Switch(
        label="启用CUDA加速",
        value=cuda_available and True,
        disabled=not cuda_available,
        active_color=THEME_PRIMARY,
    )

    cuda_row = ft.Row(
        [use_cuda, cuda_status],
        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,  # 改为两端对齐
        spacing=20
    )
        ...
        ft.Container(
            content=ft.Column(
                [
                    ...
                    section_header("2. 选择MIDI输出位置 (可选)"),
                    output_row,
                    ft.Container(height=10),
                    section_header("3. 选择转换选项"),
                    cuda_row,
